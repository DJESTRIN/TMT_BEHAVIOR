...
#sshpass -p "estrin1" ssh -n -tt $IP 'killall screen && exit; exec bash -l'
#done < IPS.txt

# Initalize all screens
while IFS=, read IP box;do
echo "SSH into " $IP " and start screen session for box " $box
sshpass -p "estrin1" ssh -n -tt $IP '[screen -dmS testscreen || echo "screen does not exist on box $box" ] && exit; exec bash -l'
done < IPS.txt

# Run a preview
while IFS=, read IP box;do
echo "SSH into " $IP " and run preview for box " $box
sshpass -p "estrin1" ssh -n -tt $IP 'screen -S testscreen -p 0 -X stuff "python ~/base/code/temp/preview.py" && exit; exec bash -l'
done < IPS.txt

# Rsync zipped data to conductor pi
while IFS=, read IP box;do
echo "SSH into " $IP " and rsync data for box " $box
sshpass -p "estrin1" ssh -n -tt $IP 'screen -S testscreen -p 0 -X stuff "rsync -var ~/test_zip.zip pi@192.168.0.147:~/video_archive/$box/" && exit; exec bash -l'
done < IPS.txt
