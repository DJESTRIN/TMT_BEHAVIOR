...
do
sshpass -p "estrin1" ssh -n -tt $IP 'tag=$(date "+%Y%m%d%H%M%S") && sudo screen -dmS $tag bash -c "echo $message; exec bash" && sudo screen -r $tag -p 0 -X stuff "bash /home/pi/base/code/temp/Record_RUN.sh\n" && exit; exec bash -l'
done < schedule.txt
